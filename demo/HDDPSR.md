---
title: "XYZ High Density Dual Port SRAM Compiler User Manual"
toc-title: "Contents"
---
...
This chapter provides an overview of the embedded memories of High Density Dual Port SRAM Compiler memory compiler.
...
+----------------+----------------------------+---------------+
| Name Segment   | Character in "XYZHDDPSR"   | Description   |
+================+============================+===============+
...
+-----------------+----------------------------------------+
| Compiler Name   | High Density Dual Port SRAM Compiler   |
+=================+========================================+
| SVT             | Default                                |
+-----------------+----------------------------------------+
| LVT             | Low                                    |
...
| Enable    | - If the BWENA/BWENB = 0,  the corresponding bit’s data is written to  |
|           | the bit cell array.\                                                   |
|           | - If the BWENA/BWENB = 1,  the corresponding bit’s data is not written |
...
| Feature    | Description                                                            |
+============+========================================================================+
| CEN-Gating | By default, the AA/AB/DA/DB/WENA/WENB/BWENA/BWENB input pins are not   |
|            | gated by the CENA/CENB pin may increase toggle power. In this case,    |
|            | CENA/CENB requires a relatively small setup time.\                     |
...
![Column Redundancy Replace Mechanism](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\Column_Redundancy_replace_mechanism.png)
...
![Column Redundancy Waveform](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\1_Column_redundancy.png)
...
The specifications of High Density Dual Port SRAM Compiler.
...
+---------------+----------------------+------------------------------------------------------------------------+
| DB [n-1:0]    | Always               | B port Data input.                                                     |
+---------------+----------------------+------------------------------------------------------------------------+
...
+---------------+----------------------+------------------------------------------------------------------------+
| BWENB [n-1:0] | Bit Write feature    | B Port Bit Write Enable. Bit Write is enabled when BWENB is Logic Low. |
|               |                      | When the Bit Write feature is included,  write operation can be        |
|               |                      | performed by enabled I/Os bits.                                        |
+---------------+----------------------+------------------------------------------------------------------------+
| WENA          | Always               | A Port Global Write Enable input. The memory is in the Write cycle     |
|               |                      | when the Global Write Enable input is Logic Low; the memory is in the  |
|               |                      | Read cycle when it is Logic High.                                      |
+---------------+----------------------+------------------------------------------------------------------------+
| WENB          | Always               | B Port Global Write Enable input. The memory is in the Write cycle     |
|               |                      | when the Global Write Enable input is Logic Low; the memory is in the  |
|               |                      | Read cycle when it is Logic High.                                      |
+---------------+----------------------+------------------------------------------------------------------------+
| CENA          | Always               | A Port Chip Enable input. The memory is enabled and can perform        |
|               |                      | read/write operations when CENA is Logic Low; the memory is disabled   |
|               |                      | when it is Logic High.                                                 |
+---------------+----------------------+------------------------------------------------------------------------+
| CENB          | Always               | B Port Chip Enable input. The memory is enabled and can perform        |
|               |                      | read/write operations when CENB is Logic Low; the memory is disabled   |
|               |                      | when it is Logic High.                                                 |
+---------------+----------------------+------------------------------------------------------------------------+
...
| Signal     | Enabled              | Description                                                          |
+============+======================+======================================================================+
| QA [n-1:0] | always               | A Port Data Output bus.                                              |
+------------+----------------------+----------------------------------------------------------------------+
...
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
| 4         | 8      | 16      |     256 |    4096 |      1 |      8 |    320 | 1.25Mb     |
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
...
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
| 8         | 8      | 32      |     512 |    8192 |      1 |      8 |    160 | 1.25Mb     |
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
...
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
| 16        | 8      | 64      |    1024 |   16384 |      1 |      8 |     80 | 1.25Mb     |
+-----------+--------+---------+---------+---------+--------+--------+--------+------------+
...
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
| Function   | CLKA   | CLKB   | CENA   | CENB   | WENA   | WENB   | BWENA   | BWENB   | DA      | DB      | QA   | QB   | Memory\    |
|            |        |        |        |        |        |        |         |         |         |         |      |      | Contents   |
+============+========+========+========+========+========+========+=========+=========+=========+=========+======+======+============+
| Idle       | L      | L      | X      | X      | X      | X      | X       | X       | X       | X       | Q-1  | Q-1  | NCM        |
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
| Disabled   | H      | H      | H      | H      | X      | X      | X       | X       | X       | X       | Q-1  | Q-1  | NCM        |
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
| Read       | H      | H      | L      | L      | H      | H      | X       | X       | X       | X       | Q    | Q    | NCM        |
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
| Write      | H      | H      | L      | L      | L      | L      | L       | L       | Data-in | Data-in | Q-1  | Q-1  | Data-in    |
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
| Write Mask | H      | H      | L      | L      | L      | L      | H       | H       | X       | X       | Q-1  | Q-1  | NCM        |
+------------+--------+--------+--------+--------+--------+--------+---------+---------+---------+---------+------+------+------------+
...
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Mode        | CENA   | CENB   | LS   | DS   | SD   |   PUR | QA   | QB   | Comments             |
+=============+========+========+======+======+======+=======+======+======+======================+
| Normal      | 0/1    | 0/1    | 0    | 0    | 0    |     1 | Q-1  | Q-1  | Normal mode          |
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Light Sleep | 0/1    | 0/1    | 1    | 0    | 0    |     1 | Q-1  | Q-1  | Xdec power gate,\    |
|             |        |        |      |      |      |       |      |      | Bitline Discharge    |
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Deep Sleep\ | X      | X      | X    | 0->1 | 0    |     0 | 0    | 0    | Periphery shutdown   |
|  enable     |        |        |      |      |      |       |      |      |                      |
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Deep Sleep\ | X      | X      | X    | 1->0 | 0    |     1 | X    | X    | Periphery wakeup     |
|  disable    |        |        |      |      |      |       |      |      |                      |
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Shut Down\  | X      | X      | X    | X    | 0->1 |     0 | 0    | 0    | Periphery shutdown,\ |
|  enable     |        |        |      |      |      |       |      |      | array shutdown       |
+-------------+--------+--------+------+------+------+-------+------+------+----------------------+
| Shut Down\  | X      | X      | X    | X    | 1->0 |     1 | X    | X    | Periphery wakeup,\   |
...
>Note: If `CENA/CENB` is Low, it must be ensured that each signal meets the timing specification relative to the clock.\
>      If `CENA/CENB` is High, there is no need to meet the timing specification for each signal.\
>      This note also applies to Power Management pins such as `LS`.\
>      When `CENA/CENB=1`, `LS` can be asynchronously asserted and de-asserted.
...
![Waveform Conventions](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\waveform_conventions.png)



![Read Cycle Timing Diagram](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\read_cycle_timing_diagram.png)




![Write Cycle Timing Diagram](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\write_cycle_timing_diagram.png)



![Light Sleep Mode Work Function Diagram](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\light_sleep_mode_work_function_diagram.png)




![Deep Sleep Mode Work Function Diagram](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\deep_sleep_mode_work_function_diagram.png)




![Shut Down Mode Work Function Diagram](../Template/SC/V02/Figure\High Density Dual Port SRAM Compiler\shut_down_mode_work_function_diagram.png)
