...
>Note: If `CEB` is Low, it must be ensured that each signal meets the timing specification relative to the clock.\
>      If `CEB` is High, there is no need to meet the timing specification for each signal.\
>      This note also applies to Power Management pins such as `LS`.\
>      When `CEB=1`, `LS` can be asynchronously asserted and de-asserted.
...
> Note: To avoid memory access during wake-up, an alternative solution is to set the `CEB` pin to the inactive state for at least the time interval of `Tflsc/Tfdsc/Tfsdc` after the falling edge of `LS/DS/SD`.
