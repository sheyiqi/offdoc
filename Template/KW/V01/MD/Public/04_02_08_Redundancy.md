...
Figure show an example of the column redundancy replace mechanism in a 8 bits column Mux memory instance. In the case, IO6 needs to be repaired. Once redundancy DFF chain be configured, redundancy scheme works. DO[7] receives the output of the redundant sub-array, DO[6] receives the output of DO[7], then replacement complete. The rest IO[0]~IO[5] receive the output of themselves.

One IO where defect is detected can be replaced by redundancy. All data flip-flop which used to configure redundancy logic to column redundant substitution. The redundant substitution of configure choose based on memory`s bit width of DI bus.

![Column Redundancy Waveform Usage]([Template Path]\[Compiler Name]\1_Column_redundancy.png)
...
![2 Column Redundancy Waveform Usage]([Template Path]\[Compiler Name]\2_Column_redundancy.png)

Figure shows the scan sequence of 2 column redundant substitution is followed by both left and right least significant bit of io address (0), then left io address increasing until most significant bit of io address (X-2) is captured, and finally left redundancy enable signal (X-1) is the last signal of scan sequence. Similarly, right io address increasing until most significant bit of io address (N-2) is captured, and finally right redundancy enable signal (N-1) is the last signal of scan sequence.
