...
> Note: To avoid memory access during wake-up, an alternative solution is to set the `CEB` pin to the inactive state for at least the time interval of `Tflsc/Tfsdc` after the falling edge of `LS/SD`.
