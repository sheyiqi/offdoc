...
[Company Name] memory compiler provides the enable signal through the `EMCE` pin. The `EMCE` pin allows for the selection between internal default margin control and external margin control. When `EMCE` is low, the instance uses the default self-timing control.

There are `Extra Margin Control` bus pins available in the memory compiler. `EMC` are used to trade-off between speed and robustness (`yield`). It is required that external access to all `EMC` pins, including `EMCE`, is provided for debug and yield analysis purposes. 

When `EMCE` is high, instance margin could be adjusted by the extra input pins. 
The margin sequentially increases as `EMC` sequentially decrease from All 1 to All 0, but the access time and cycle time will be sequentially enlarged as the pins driven from All 1 to All 0.

The `Extra Margin Control` settings are user-adjustable. The user-side `EMC` interface is uniformly encoded, ensuring that all compilers have the same default `EMC` value. Users can configure the settings from the options shown in the table below according to their application requirements.    
